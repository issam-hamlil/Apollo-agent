...
class FixerAgent(
    ...
    fun execute(state: GraphState): GraphState {
        ...
        val report = AgentReport(
            ...
            details = buildString {
                append("Applied targeted fixes to $fixedCountInt module(s). $cappedCountInt module(s) reached max retry cap ($maxRetriesPerModule).")
                if (configErrorModules.isNotEmpty()) {
    ...
    internal fun gatherFailureLogsForModule(
        ...
        if (moduleDiagnostics.isNotEmpty()) {
            ...
            if (generalErrors.isNotEmpty()) {
                ...
                sb.appendLine("=== PROJECT COMPILATION FAILURE LOGS ===")
                verification.compilerErrors.forEach { sb.appendLine(" - $it") }
            }
        }
    ...
    private fun tryFixWithLLMResult(
        ...
        val systemPromptStr = """
            ...
            3. Fix ALL reported errors in the failure logs for $className:
               ...
               - If a method return type mismatch occurs (e.g. expected 'String', actual 'String?' when returning null), update the method return type signature to be nullable (e.g. `fun foo(...): String?`) so null return values are allowed.
               - If a characterization test failure reports: expected 'null', got '', ensure the function returns null (not empty string) when the input is null or empty, and ensure the return type is nullable `String?`.
        ...
        return try {
            val rawOutput = LlmConfig.callLlmWithFallback(userPromptStr, systemPromptStr, temperature = 0.1, moduleName = className, attemptCount = attemptCount)
