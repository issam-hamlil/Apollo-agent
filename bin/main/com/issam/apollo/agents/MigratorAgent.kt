...
class MigratorAgent(
    ...
    private fun tryMigrateWithLLMResult(
        ...
        val systemPromptStr = """
            ...
            3. Apply Kotlin null safety (?. , ?: , requireNotNull), data classes for POJOs, object/extension functions for utilities, and collection functions (filter, map, find). If a Java method can return null (e.g. `if (str == null) return null` or `if (isEmpty(str)) return str`), declare its return type as nullable (e.g. `String?`) and preserve null return behavior.
